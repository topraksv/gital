 ...
 */
export function restockDue(purchases: readonly Purchase[], onList: readonly { name: string }[], now: Date): Restock[] {
  ...
  for (const [key, bought] of byProduct) {
    if (bought.length < RHYTHM_FROM) continue;
    const times = bought.map((purchase) => Date.parse(purchase.boughtAt));
    const gaps = times.slice(1).map((time, at) => times[at]! - time);
